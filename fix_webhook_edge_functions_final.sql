/*
  # Corrección Final de Problemas con Webhook de IA y Edge Functions

  Este script corrige los problemas restantes identificados:
  1. Asegura que el contenido del mensaje se incluya en el payload enviado al webhook de IA
  2. Corrige el problema de los mensajes de agentes que se están tratando como mensajes de clientes
  3. Corrige el error de sintaxis en la función
*/

-- 1. Modificar la función notify_message_webhook para incluir el contenido en el payload
...
-- 2. Recrear el trigger para asegurarse de que esté usando la función actualizada
...
-- 4. Verificar si hay mensajes de agentes que se están tratando como mensajes de clientes
...
BEGIN
  ...
  VALUES (
    ...
    'CORRECCIÓN FINAL CON CONTENIDO: Mensaje de prueba para webhook de IA ' || NOW(),
    'client',
    test_client_id,
    'text',
    'sent',
...
-- 6. Instrucciones para corregir las Edge Functions
DO $$
BEGIN
  RAISE NOTICE '
  INSTRUCCIONES PARA CORREGIR LAS EDGE FUNCTIONS:
  
  1. Edge Function messages-outgoing:
     - Esta función procesa mensajes de agentes y los envía a n8n
     - Problema: Está actualizando los mensajes en la base de datos, lo que podría estar causando conflictos
     - Solución: Revisar y modificar la función según las instrucciones en el archivo README-edge-functions.md
  
  2. Edge Function messages-incoming:
     - Esta función procesa mensajes entrantes
     - Problema: Podría estar interfiriendo con el procesamiento de mensajes
     - Solución: Revisar y modificar la función según las instrucciones en el archivo README-edge-functions.md
  
  Recuerda verificar los logs de Supabase después de aplicar las correcciones para confirmar que todo funciona correctamente.
  ';
END;
$$;
