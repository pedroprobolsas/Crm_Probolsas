# Script simplificado para aplicar la solución completa del webhook de IA
# Este script ejecuta directamente el archivo SQL fix_ia_webhook_completo.sql

# Configuración
$sqlFilePath = "fix_ia_webhook_completo.sql"
$logFilePath = "fix_ia_webhook_completo_log.txt"
...
$continue = Read-Host "Este script aplicará la solución completa para el problema del webhook de IA. ¿Desea continuar? (S/N)"
...
Write-Host ""
Write-Host "Aplicando solución completa para el problema del webhook de IA..." -ForegroundColor Yellow
...
try {
    ...
    if ($result -match "ERROR:") {
        ...
        Write-Host "  ERROR AL APLICAR LA SOLUCIÓN" -ForegroundColor Red
        Write-Host "====================================================" -ForegroundColor Red
        Write-Host ""
        ...
        Write-Host ""
        Write-Host "Si sigues teniendo problemas, es posible que necesites revisar y modificar las Edge Functions" -ForegroundColor Cyan
    ...
    Write-Host "4. Aplica la solución manualmente copiando y pegando el contenido del archivo" -ForegroundColor Cyan
    Write-Host "   $sqlFilePath en la consola SQL de Supabase." -ForegroundColor Cyan
}
